...
# Delivery table ABCDS, wider from. quick read
...
# =============================================================================
os.getcwd()
df = pd.read_csv('delivery table ABCDS 2018-05-31_Tcleaned2.csv')
...
temp_dates['form_add'] = pd.to_datetime(df['form_add'])
temp_dates['form_edd'] = pd.to_datetime(df['form_edd'])
...
## SEASON OF EDD, SEASON OF INITIATION
...
cols = ['index1', 'anyYes20daysPriorToBirth_form_has_bank_account', 'numVisits_20daysPriorToBirth', 'probs']
col = 'index1'
SHOW_GRAPHS = True
p_test[TARGET] = Y
#iterate through
for col in cols:
    #prepcumsum
    x = pd.DataFrame(p_test.sort_values(col,ascending=False)[[col,TARGET]]) 
